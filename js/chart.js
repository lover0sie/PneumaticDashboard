...
/*------------- Init variables ----------- */
    ...
    function msToPct(ms, window){
    const span = window.max - window.min;
    return ((ms - window.min) / span) * 100;
    }
    ...
    if (!currentRows.length) {
        alert("No data loaded for this day.");
        return;
    }
    const csv = rowsToCsv(currentRows);
    downloadTextFile(`segments_${currentDateStr || "day"}.csv`, csv);
    });
    ...
    function renderTable(rows, window){
      ...
      for (const r of rows){
        ...
        bar.className = "barWrap"; // barWrap is width:100% in CSS (full scale)

        const plot = document.createElement("div");
        plot.className = "barPlot";
        bar.appendChild(plot);
        ...
        for (const s of r.segments){
          ...
          // segments go into plot
          plot.appendChild(segDiv);

        }
        // ticks go into plot (so they align with segments)
        renderBarTicks(plot, window);
    ...
    function dayStartEndFromDateInput(dateStr){
    ...
    function renderBarTicks(barEl, window){
      barEl.querySelectorAll(".barTicks").forEach(x => x.remove());

      const ticks = document.createElement("div");
      ticks.className = "barTicks";
